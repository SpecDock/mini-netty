...
### 5. 🌊 线程局部内存池与零争用架构 (Thread-Local Memory Pool)

针对极高频的网络 I/O 读写场景，框架内置了专属的池化内存分配器 `PooledByteBufAllocator`。区别于传统基于全局锁或 CAS (Compare-And-Swap) 自旋指令的并发内存池，本项目采用了**线程封闭 (Thread Confinement)** 的极简设计哲学：

* **EventLoop 专属绑定与零争用 (Zero-Contention)**：
    分配器实例与单一 `EventLoop` (Worker 线程) 强绑定。基于 `LinkedList` 构建的底层池化队列 (`directBufferPool` / `heapBufferPool`) 运行在绝对的单线程上下文中。这种架构彻底摒弃了多线程环境下的同步锁竞争与 CPU 内存屏障开销，将物理内存的调度时钟周期压缩至极致。
    

* **按需动态分配与游标重置 (Dynamic Allocation & Sanitization)**：
    提供 `allocate()` 与 `recycle()` 标准流转语义。分配时优先从双向链表中复用已存在的 `ByteBuf` 实例，消除 `ByteBuffer.allocateDirect()` 带来的系统态陷入开销；在归还阶段，强制执行 `buf.reset()` 重置物理读写游标，从根源上阻断不同 TCP 报文间的数据串扰 (Data Leakage)。

* **高水位边界防御 (High-Watermark Defense)**：
    内置严格的池容量防线 (`MAX_POOL_SIZE = 1024`)。在应对极端的突发网络洪峰 (Traffic Spike) 时，若归还的缓冲区数量突破物理上限，分配器会智能触发降级销毁策略，直接调用 `buf.release()` 释放底层物理内存。此机制有效防止了池化容器本身的无限膨胀，构筑了抵御系统级 OOM (Out Of Memory) 的坚固堤坝。

* **Double Free 探活防御 (Liveness Probing)**：
    在执行内存回收进入缓冲池之前，利用 `buf.ensureAccessible()` 触发底层的并发状态检查机制。若该 `ByteBuf` 已经被外部逻辑非法越权释放（触发 `IllegalStateException`），回收逻辑会被立即拦截，构建了防范 Use-After-Free 与 Double Free 引发 JVM 崩溃的最后一道安全底线。
  
### 6. 💓 事件驱动的心跳保活与连接拓扑守护 (Heartbeat & Keep-Alive)

针对分布式网络环境下极其复杂的“假死连接”与“僵尸节点”场景，框架内置了一套高可靠、非阻塞的心跳监测机制。区别于固定频率的暴力轮询，本项目实现了按需触发的空闲监测策略：

* **基于 Idle 状态机的异步触发 (Event-Driven Idle Detection)**：
引入 IdleStateHandler 作为 Pipeline 前置哨兵，通过底层 HashedWheelTimer (时间轮) 追踪每一次物理 I/O 活动。仅当通道读/写空闲达到预设阈值（HEARTBEAT_TIMEOUT_MS）时，才会精准触发 UserEvent。这种设计避免了在活跃连接上执行冗余的心跳扫描，显著降低了 EventLoop 的无效调度开销。

* **协议层帧标识分离 (Protocol-Level Frame Decoupling)**：
重构了基础通信协议栈。在 LengthField 长度域后置位引入 FrameType (1-Byte) 标识位。实现了业务帧 (Business Data) 与 信令帧 (Ping/Pong) 在解析层面的物理隔离。心跳拦截器在 StringDecoder 前置位执行拦截，确保心跳信令在 Pipeline 中被即时消费（Consume），避免了控制流报文对业务处理逻辑的非必要侵入。

* **双端失效对冲与精准断连 (Asymmetric Failure Detection)**：
实现了服务端“读超时剔除”与客户端“写超时探测”的非对称协同。服务端作为连接的最终仲裁者，在检测到心跳丢失后执行 ctx.close() 触发物理连接熔断；客户端则作为探测者，在连接空闲时自动注入探测报文。这种机制确保了连接状态在物理链路抖动时能迅速达成全局最终一致性。

* **时间轮任务的生命周期联动 (Lifecycle-Synced Task Eviction)**：
深度联动 ChannelInactive 生命周期。当连接发生异常或被主动剔除时，全局时间轮盘监测器会立即撤销（Cancel）该 Channel 关联的所有异步调度任务。通过强引用的即时核销（Unbinding），确保失效的 NioSocketChannel 对象及其关联的缓冲区能够被 JVM 立即识别为可回收状态，从根本上杜绝了因心跳监测机制引发的内存泄漏 (Memory Leak) 与任务堆积风险。
